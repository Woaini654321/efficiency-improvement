# Claude Code 项目提效实战培训手册

> **面向对象**：全体开发人员 + 公司高层
> **讲师背景**：15 年全栈，使用 Claude Code 完成 3 个 0→1 项目（思政项目、baseProject 产品线、web-tool 工具平台）+ 1 个存量项目提效（工单系统）
> **手册定位**：既是培训讲义，也是团队长期查阅的 SOP。所有案例均来自真实项目，所有命令均可直接照做。
...
- **Part A 认知与价值（高层重点）**
  - 第 0 章 给高层的 3 分钟：ROI 一页纸
  - 第 1 章 认知对齐：AI 辅助开发 = 人机协作工程
- **Part B 个人提效基本功（全员）**
  - 第 2 章 5 分钟上手 + 常见姿势纠错
  - 第 3 章 核心工作流六步法
  - 第 4 章 提效技巧库
- **Part C 岗位视角与全流程协同（重头戏）**
  - 第 5 章 七大岗位提效地图
...
### 0.1 三个真实数据点（全部来自我们自己的项目）

| 项目 | 类型 | 传统模式预估 | AI 协作实际 | 备注 |
|---|---|---|---|---|
| **NCM 音频转换器**（baseProject 分支） | 0→1 纯前端产品 | 设计+开发+测试约 2~3 周 | **设计文档当天定稿，实施当天交付**：8 个模块 TDD 全部落地，36 个单测通过、类型检查零错误 | 采用"子代理驱动开发"模式 |
| **web-tool 工具平台** | 0→1 全栈平台 | 前后端联调通常 1~2 周+多轮返工 | **两天合并 3 个 PR**：主站+SEO 引擎（131 测试）→ 音频栈+47 个落地页（206 测试）→ 完整 Spring Boot 后端（前后端 294 测试）；**前后端契约先行，联调零返工** | 一人完成前端+后端+部署产物 |
| **工单系统**（存量项目） | 存量提效 | 全面业务审计需外部审计或数周专项 | **入场第 1 天完成项目配置；第 2 天六角色深度走查产出带证据的漏洞文档**（挖出 3 个 P0 安全红线 + 多个 P1）；随后按组并行修复，单组 11 任务一天闭环、322 个单测全绿 | AI 发现了长期存在、一直无人发现的认证绕过漏洞 |
...
- **提速**是表象；**提质**（六角色走查挖出 JWT 伪造漏洞）和**资产化**（经验写进 CLAUDE.md/Skill，新人第一天就继承团队最佳实践）才是组织级复利。
- **降门槛**改变人才结构：一个熟练使用者可以覆盖过去 2~3 个角色的产出（见第 10 章案例）。

### 0.3 高层最关心的四个问题，本手册的回答

| 问题 | 答案在 | 一句话结论 |
|---|---|---|
| 到底能省多少？ | 第 14 章 | 别信通用百分比，用我们自己的度量体系跑 1 个月试点拿真数据 |
...
传统开发中真正写代码的时间占比其实不高，大量时间花在：读懂旧代码、查资料、写样板代码、联调返工、写文档、开对齐会。Claude Code 压缩的正是这些"非创造性时间"——在我们的案例里：读懂存量系统从数周专项变成 2 天走查（工单系统）、前后端联调返工直接归零（web-tool）、日报从每天 15 分钟变 1 分钟（思政）。具体倍数因场景差异很大，所以第 14 章坚持"用自己的基线测"而不引用通用百分比；但方向是确定的：体感提效远超"代码补全工具"。
...
> 🧩 **不习惯终端？** VS Code / JetBrains 官方插件提供同样的能力，diff 直接在编辑器里审、文件改动实时可见。习惯 IDE 的同事从插件入门，推广阻力小一半——能力和 CLI 完全一致，本手册所有方法通用。
...
**① 需求澄清（Brainstorm）+ 评审门禁** — 让 AI 一次只问一个问题，把模糊需求磨成明确规格；收口时让 AI 以开发/测试视角把需求"挑一遍刺"（歧义、缺失异常流、边界未定义）再冻结。
> 实例：NCM 转换器立项时用"四问定架构"——转换放前端还是后端？要不要后端？输出什么格式？仓库怎么组织？——四个大方向确认后，后续实施**零方向性返工**。

**② 设计方案（Spec）+ 多角色评审 1~3 轮** — 产出设计文档存入 `docs/specs/`，包含架构、数据流、错误处理、测试策略；随后多角色（PM/架构/前后端/测试，涉敏加安全）评审，**每轮修完 must-fix 再评下一轮，直到各角色一致放行才定稿**。设计文档是后续一切环节的"事实锚点"。
> 实例：工单系统组D 运营能力的 spec 经**两轮四角色评审**才定稿；思政项目知了知识库授权矩阵经**两轮三方评审**落地。

**③ 实施计划（Plan）+ 多角色评审 + 人工批准** — 计划拆成带验收标准的任务块，先过一轮多角色评审（重点：任务边界、依赖顺序、验收标准是否可执行），再交人批准。**人只需要审计划，不需要盯每行代码**——这是杠杆率最高的一次人工介入。
> 实例：思政项目数据中心需求 **spec 和 plan 双阶段都做了多角色评审**，每一轮都抓到真 blocker（spec 轮：漏 @EnableAsync、ThreadLocal 跨线程竞态；plan 轮：403 走 HTTP-200+R.code 约定、tenantType 大小写）——**同一批问题若流到编码后才发现，返工成本至少放大十倍**。

**④ 分块实施（TDD 小步）+ 逐块评审** — 每块先写测试再实现，块与块之间可独立验证；每块完成即评审（SDD 模式：实现代理+评审代理接力，见第 8 章），本块 must-fix 清零才进下一块。
> 实例：NCM 转换器 8 个模块块、工单系统组B 11 个任务，全部逐块落地逐块评审。

**⑤ 全量终审** — 所有块完成后做一次整体多角色评审：跨块一致性、契约对齐、遗漏项（逐块评审只看局部，终审看全局）。
> 实例：工单系统组B 终审 READY_TO_MERGE、组D "终审零必修"；web-tool 每 Chunk 双阶段审查后仍有全量终审。
...
> ⚠️ 关键教训：**不同项目红线可能完全相反**。思政项目（内网仓库）允许配置文件提交凭据；工单系统严禁任何明文密钥、必须 AES-GCM 加密入库。CLAUDE.md 按项目写死，AI 就不会拿 A 项目的习惯祸害 B 项目。
...
> ⚠️ **纠错一个流传极广的错误说法**：网上（甚至 AI 自己）常推荐建 `.claudeignore` 文件——**它不是官方机制，实测拦不住文件读取**。它给你的只是"已经排除了"的错觉，比没有更危险。我们早期项目也建过，本手册已统一纠正——别把它写进你的团队规范。

### 4.4 斜杠命令：把重复操作固化（★★★★）

`.claude/commands/` 下放 Markdown 文件即成为团队共享命令：
...
> 实例：思政项目的 `/daily-report` 把每天 15 分钟写日报变成 1 分钟确认。
...
> 实例：baseProject 授权无人值守后，整个转型实施"设计当天定稿、当天交付"。**前提是红线写清楚 + 事后全量验证**，不是裸奔。
...
**落地要点**：CI 中的 AI 权限要**最小化**（只读+产出报告，不自动改码合并）；产出物统一落到固定路径供人复核；先从"评审建议"类零风险场景起步，跑稳了再扩权。计费上有两条路：`claude setup-token` 生成长期令牌复用现有订阅额度（与交互用量共享限额），或走 API Key 按量计费——试点期实测用量再定。这条线是对高层讲 ROI 时的加分项——**同一套工具链，产出时间从 8 小时/天变成 24 小时/天**。
...
**提效亮点**：UI 选型"用看的不用猜的"——NCM 转换器美化时让 AI 生成 4 套自包含 HTML 样张，产品/用户直接选中意的一套再落地，避免"做完了说不是这个感觉"。
...
**提效亮点**：**契约先行**。web-tool 项目先写 `api-contract-v1.md`（状态码、错误格式、文件名编码全部定死），前后端各自实施后端到端实测**零返工**。
...
| 样式一致性 | 主题/设计系统抽取与批量套用（web-tool 全站重皮实例，教训：**迁 CSS ≠ 还原外观，组件结构必须匹配主题选择器**） |
| 状态管理与类型 | 前端 35 处 any 一次性消除+新增 10 个接口类型（思政项目实测） |
...
> 实测效果：思政项目 spec+plan 双阶段评审，**每一轮都抓到会导致返工的真问题**；工单系统每个任务组固定"知会 Architect 契约变化"收尾，跨端契约从未失配。
...
> 这是从"一个 AI 助手"升级到"一支 AI 团队"的分水岭，也是本次培训的进阶重点。
...
> ⚠️ 实践约定（工单系统沉淀）：派子代理时**显式指定模型**（如 Opus），避免静默降级到低档模型影响产出质量——质量与成本的平衡点应由团队显式决定。

### 8.2 多角色评审：子代理的最高性价比用法
...
真实数据：
- NCM 转换器：8 块 SDD 当日交付，36 单测全绿
- 工单系统组B：11 任务 SDD 双车道（两条流水线并行）+ 逐任务评审 + 终审 READY_TO_MERGE，一天闭环
- 工单系统组D：9 任务 SDD 全闭环，"终审零必修"
...
> 换电脑实证：Mac→Windows 迁移时，skills + memory 全部随身迁移，四个项目的经验资产零丢失。**员工的隐性经验第一次变成了可迁移、可继承、可审计的显性资产**——这一点对高层的意义不亚于提效本身。
...
**真实案例就是本手册自己**：Mac→Windows 换机后，从旧 `.claude` 文件夹里完整恢复了思政/baseProject/web-tool 三个项目的全部项目记忆、踩坑教训和 skill——本手册第 10 章的案例数据全部来自这次恢复。
...
### 10.1 案例一：思政项目（0→1 全栈，含技术栈整体迁移）

**背景**：教育行业项目（学校 AI Box / 长宁项目），monorepo：Web 前端（Vue3+TS+AntDV）+ H5 + 后端。

**关键战役**：
1. **0→1 搭建**：单人完成前端+H5+后端+部署整条线
2. **安全加固冲刺**（一轮完成）：API 密钥从前端全部移到后端代理、XSS 全量 DOMPurify 消毒、前端 35 处 any 消除、前后端 37 个测试补齐——传统模式这是一个"排期两周的专项"，实际一个冲刺收口
3. **NestJS→Spring Boot 整体迁移**：公司技术栈统一要求。设计文档先行（迁移设计+实施计划），8 大业务模块迁移，双端并行期 API 路由 100% 一致，前端无感切换
4. **数据中心/活跃度统计需求**：spec+plan 双阶段多角色评审（PM/架构/前后端），抓到漏 @EnableAsync、ThreadLocal 跨线程竞态、403 走 HTTP-200+R.code 契约约定、tenantType 大小写等真 blocker

**可讲的经验**：
- 强制六步工作流写进 CLAUDE.md，是后面所有项目的流程母版
- 多角色评审第一次展现威力：**评审几分钟 vs 调试几天**
- /daily-report 自动化：日报从 15 分钟/天 → 1 分钟确认

### 10.2 案例二：baseProject 分支产品线（一个仓库长出多个产品）

**模式**：`main` 分支 = 企业级跨平台 IM 模板（RN+Node+PostgreSQL 完整资产）；新产品 = 开新分支转型，历史资产保留可随时恢复。

**战绩**：`feature/NCM-mp3` 分支从 IM 模板转型为纯前端音频转换器：
- Day 1：四问定架构 → 设计文档定稿 → **8 块 SDD 当日全部落地**（36 单测、类型零错、构建通过）
- Day 3：扩展为通用音频转换器：可插拔引擎抽象 + 47 个 SEO 组合落地页构建期预渲染，68 单测、覆盖率 ≥87%，两轮评审 must-fix 全修

**可讲的经验**：
- **无人值守模式**的正确打开方式：显式授权范围（可自动选型/装依赖）+ 保留硬红线（不 push/不删库）+ 事后全量验证
- **真实样本验证教训**：NCM 3.0+ 格式偏移坑——自造测试夹具测不出真实文件的容器差异，上真实样本后 80/80 通过才收口。**AI 的自测可能构造出"自洽但脱离现实"的验证，关键路径必须注入真实数据**
- UI 选型样张化：4 套 HTML 样张让干系人"用看的选"

### 10.3 案例三：web-tool 工具平台（资产组合 + 契约驱动，两天成型）

**模式**：pdf-tools 的插件式架构（内核）+ baseProject 的视觉主题（皮肤）→ 合成新平台。**已有项目资产像积木一样组合**。

**战绩**：设计定稿次日，一天合并 3 个 PR：

| PR | 内容 | 测试 |
|---|---|---|
| S0 主站地基 | 4 工具 + SEO 引擎（双语正文+JSON-LD 预渲染）+ 合规组件 | 131 |
| S0.5 音频迁移 | ffmpeg.wasm 栈 + 47 组合落地页（构建 115 页） | 206 |
| S1 后端 | 完整 Spring Boot 3（反馈 API+文档转换+管理后台+限流+部署产物） | 后端 69 + 前端 225 |

**可讲的经验**：
- **契约先行 = 前后端零返工**：`api-contract-v1.md` 先定死（成功二进制/失败 JSON 互斥、429 Retry-After、RFC5987 文件名），双方照契约各自实施，端到端 curl 实测一次对齐
- 每 Chunk 双阶段审查 + 全量终审的质量节拍
- 分阶段交付：每阶段一个 PR 一个可验收里程碑，进度对外完全透明

### 10.4 案例四：工单系统（存量项目提效——对多数团队最有参考价值）

> 前三个案例是 0→1，但多数同事日常面对的是存量系统。这个案例回答："接手一个跑了很久的系统，AI 能干什么？"
...
| | 思政 | baseProject | web-tool | 工单系统 |
|---|---|---|---|---|
| 类型 | 0→1 + 迁移 | 分支产品线 | 资产组合 | 存量提效 |
| 核心方法 | 六步法+多角色评审 | 无人值守+SDD | 契约驱动+分阶段 | 走查+分组并行 |
| 标志性数据 | 35 any 清零/37 测试一冲刺 | 设计当天→交付当天 | 两天 3 PR/零返工 | 3 天走查+修复 3 组并行 |
| 最大启示 | 流程即质量 | 授权即速度 | 契约即并行 | 认知即入场券 |
...
4. **各项目独立成文，禁止跨项目复制粘贴**（真实教训：思政项目"凭据可提交"与工单系统"严禁明文密钥"完全相反，抄错红线就是事故）

### 11.3 一致性检查的自动化（Hook + CI）

| 层 | 机制 | 例子 |
|---|---|---|
| 会话内 | Hook 拦截 | 提交前强制 lint/format；危险命令二次确认 |
...
| AI 特有风险 | 防线 | 真实案例 |
|---|---|---|
| **自洽但脱离现实的验证**（自造测试数据测不出真实差异） | 关键路径强制真实样本验证 | NCM 3.0 格式坑：自加密夹具全绿，真实文件乱码；换 80 个真实样本验证后才收口 |
...
经验法则：一个研发人员月成本数万元，订阅费约为其 1%~3%。**只要每月为每人节省半天以上，即为正收益**——从我们的案例看（日报自动化每天节省 14 分钟一项就够了），这个门槛非常容易越过。真实的争议从来不是"值不值"，而是"如何让不熟练的人也达到熟练者的收益水平"——这正是本培训和第 15 章路线图要解决的。
...
**阶段 2 规范化**：把试点沉淀为《团队 AI 协作规范 v1》（CLAUDE.md 模板、红线清单、门禁标准、本手册附录的命令库）；全员培训用"1 天课程 + 2 周结对"模式；出口考核用**附录 D 基本功清单**（10 项实操，结对演示 8/10 通过）。
...
| 阻力 | 心声 | 应对 |
|---|---|---|
| "AI 写的代码我不放心" | 质量担忧 | 展示门禁机制（第 12 章）：AI 代码过的关卡比人写的更多；用工单系统安全走查战绩反转认知 |
| "我自己写更快" | 熟练者的真实感受（前两周） | 承认学习曲线；结对让他看到熟练者的真实速度；从"读代码/写测试/查资料"等无争议场景切入 |
| "会不会替代我" | 职业焦虑 | 讲清楚人机分工（1.2 节）：决策、审查、验收的价值更高了；公司立场：AI 提升人的杠杆，不做减员工具（需高层明确表态） |
...
- [ ] 团队：建 `.claude/commands/` 放入第一个共享命令
...
> **手册结束。** 配套《宣讲提纲（90 分钟版）》见同目录。
> 本手册所有案例数据来自真实项目记录（思政项目 / baseProject / web-tool / 工单系统），可现场演示。
