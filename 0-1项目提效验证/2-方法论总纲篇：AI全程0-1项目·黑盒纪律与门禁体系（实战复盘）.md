...
> 📚 **《AI 全程 0→1 全栈项目实战》系列 · 第 2 篇 / 共 7 篇**
...
![个人盲猜有误，AI 按官方文档纠正](assets/B011-0.png)
*实战现场：开局按个人猜测推进即出偏差——抛弃猜测，直接按官方文档推进*

![大模型纠正前期准备工作](assets/B017-0.png)
*大模型帮我纠正前期准备工作，证明个人盲猜有误——这就是"把 skill 当权威"的第一课*
...
![AI 编译发现后端问题并解决](assets/B052-0.png)
*实战现场：AI 编译发现后端问题 → 自己修复——"自修复门禁铁律"在骨架阶段就已生效*

![前端发现问题、解决问题](assets/B056-0.png)
*前端同样：发现问题 → 解决问题,不把红字抛给人*
...
![发现前端连的是测试环境](assets/B096-0.png)
*实战现场：页面 404 排查中发现重大问题——前端连的是测试环境而非本地服务端，前后端根本没联调*

![让大模型分析前端报错原因](assets/B090-0.png)
*前端实际运行报错时，让大模型系统化分析原因，而不是盲改配置*

### 红线 5 · SSO 是框架自带，别乱配

- 框架已集成 SSO（UAA/OAuth2.0）。**除非专门调试 SSO，否则不要动 `VITE_DEV_SSO_*` 等配置**。localhost 拿不到 token 是回调/跨站 cookie 问题，不是"要你去手搓登录态"。

![发现存在两套登录](assets/B105-0.png)
*实战现场：发现存在两套登录——一套 SSO、一套默认登录*

![AI 探索 SSO 卡点](assets/B108-0.png)
*从 AI 探索来看，直接使用 skill 无法跑通 SSO（咨询同事后改用默认登录）——这是 skill 本身的卡点，需要加强约束，也是给套件团队的真实反馈*

![通读 SSO 文档确认需求](assets/B098-0.png)
*开发者描述模糊时，让大模型通读 SSO 文档做进一步说明再确认需求*

### 元规则 · 冲突先问，不回则按 skill

> **skill 与用户指令冲突时，先问人；人不回复，则严格按 skill 执行。不自由裁决。**

![重要的记忆提示是有效数字资产](assets/B092-0.png)
*实战现场：AI 把排障结论沉淀为项目记忆——这些"数字资产"让后续会话不再复踩同一坑*
...
![多角色评审不能少](assets/B114-0.png)
*实战现场：虽然是拉取的框架骨架，多角色评审也不能少*

![上下文过高时重开窗口](assets/B117-0.png)
*上下文过高、且要切换到需求处理时——`/compact` 或干脆重开窗口，防止无关上下文拖垮 AI 精准度*
...
![重要提效点](assets/B076-0.png)
*实战现场：一次解决、终身受益的提效点——把常见启动问题的修复固化下来*

![AI 建议拆分 git 提交](assets/B081-0.png)
*AI 顺手检查出临时/日志文件会误入仓库，并建议把当前改动拆成 2 次提交让日志更清晰——节省非业务开发时间*
