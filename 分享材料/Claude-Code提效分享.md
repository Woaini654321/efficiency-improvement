# Claude Code 项目提效实战分享



## 阅读路线（先看这里）

- **只有 10 分钟**：第 1 章 →「开工路线图」→ 16.4 行动清单
- **开发者（主线）**：Part A → Part B → Part C → Part D → 第 13、14 章（质量与红线，人人适用）；附录 A/B 日常备查
- **团队负责人 / 管理者**：第 1、2 章 → 11.3/11.4 案例速览 → Part E → Part F
- **非研发岗（PM / 产品 / 测试）**：第 1、2 章 → 第 6 章找自己岗位那节 → 5.4 日报自动化 → 附录 C 岗位提示词模板 → 16.4 行动清单（Part B 其余命令行内容可跳过）
- **环境还没装好**：先按《0-1 项目提效验证 · ①开箱姿势篇》搭好环境（含内网代理、公司 skill 三件套），再回到 3.1

## 全文脉络（一张图）

```mermaid
mindmap
  root((Claude Code<br/>提效实战))
    🧭 A 认知与价值
      智能体观：只有模型人人相同
      诚实版提效账
      开工路线图：前三天
...
- **Part A 认知与价值**
  - 第 1 章 3 分钟看清价值：ROI 一页纸（含"诚实版提效账"）
  - 第 2 章 认知对齐：你的开发工具是一个智能体
  - 开工路线图：拿到项目的前三天
- **Part B 个人提效基本功**
  - 第 3 章 5 分钟上手 + 姿势纠错 + 企业框架黑盒纪律
  - 第 4 章 核心工作流六步法
  - 第 5 章 提效技巧库
- **Part C 岗位视角与全流程协同**
  - 第 6 章 岗位提效地图（7+1 个岗位）
  - 第 7 章 全流程不跑偏：单一事实源机制
  - 第 8 章 事实沟通：用可验证的产物说话
  - 第 9 章 Subagent 与 Agent Teams 实战
  - 第 10 章 Harness 自我进化机制：让工具越用越懂你
- **Part D 真实案例复盘**
  - 第 11 章 双主案例：商机平台（0→1）与工单系统（存量）+ 早期案例速览
- **Part E 团队协作 · 质量 · 治理**
  - 第 12 章 团队一致性机制
  - 第 13 章 质量保障
  - 第 14 章 安全合规红线
...
> 🧭 **本部分回答**：提效是真的吗？账怎么算？为什么同一个工具，不同人用结果天差地别？

## 第 1 章 3 分钟看清价值：ROI 一页纸

> 📌 **本章一句话**：提效是真的，账要算诚实——提在具体环节，成本也花在具体环节。

### 1.1 三个真实数据点（全部来自我们自己的项目，按时间倒序，只列可核实的绝对事实）
...
| **某政项目**（已上线） | 0→1 三端 + 技术栈整体迁移 | 一人交付 Web 管理端 + H5 + Java 后端三端；后端整栈换语言（Node→Java）**设计文档当天成文并驱动迁移**：8 个业务模块迁移期**两端并行、API 路由 100% 一致，前端零改动** | 跨语言重写传统预估 1.5~2 人月且必然出现接口漂移；迁移动机正是"统一到公司 Java 技术栈" |

> 更早的 NCM 音频转换器（8 块 SDD 当日交付）、web-tool 平台（契约先行两天 3 个 PR 零返工）见 11.3 案例速览。
...
- **提质有两种形态**：存量项目里是"挖出人没发现的问题"（工单系统六角色走查挖出 JWT 伪造漏洞）；0→1 项目里是"在流到下游之前拦住问题"——商机平台五角色评审在联调前抓到**"方案匹配"功能前后端都没做**的 P0（假数据写死在页面里、没有接口文件，端点盘点看不见，只有评审能抓到）；某政项目一轮安全加固冲刺清零四类问题、测试从 0 补到 37 个用例。
- **降门槛**改变人才结构：商机平台的实验设定就是证明——一名对公司开发环境不熟悉的开发者，零手写代码完成 0→1 全栈交付；某政项目一人交付三端。**一个熟练使用者可以覆盖过去 2~3 个角色的产出**（见第 11 章案例）。

### 1.3 提效在哪里，成本花在哪里（诚实版）

> 本文档不使用"总体快了 N 倍"这类无法核实的说法——下面两笔账来自商机平台 0→1 全程实测：提效提在具体环节，成本也花在具体环节。
...
```mermaid
pie title 早期开销构成（来自商机平台复盘，比例为示意）
    "AI 自作主张翻车后的返工" : 50
    "多角色并行的 token 开销" : 30
    "黑盒取证的一次性投入" : 20
```

1. **AI 自作主张翻车后的返工**——前期姿势不对时最贵，约占早期开销的一半。**这正是本文档整套方法论要消掉的**（3.4 黑盒纪律 / 第 4 章门禁）；
2. **多角色 subagent 的并行 token 开销**——用"只喂最小上下文 + 知道何时停止评审"控制（4.4 / 5.10）；
...
| 问题 | 答案在 | 一句话结论 |
|---|---|---|
| 到底能省多少？ | 1.3 + 第 15 章 | 省在环节、不在总量：实测首个模块打穿成配方后，后续模块投入降到约 1/5 且一次编译通过；成本上订阅费约为人力成本的 1~3%，每人每月省出半天即回本，仅日报自动化一项就已越过回本线。代价是早期返工约占开销一半——这部分靠 3.4/第 4 章的方法消掉 |
| 怎么推广不翻车？ | 第 16 章 | 试点→规范→规模化三阶段，每阶段有明确门禁 |
| 代码/数据安全吗？ | 第 14 章 | 有明确红线机制（密钥、权限模式、审计），且已在工单系统验证 |
| 质量会不会下滑？ | 第 11/12 章 | 一致性靠机制（共享配置+门禁）不靠自觉，质量线反而比纯人工更严 |

### 1.5 适用边界：这套打法不万能（诚实账的最后一笔）

以下场景现阶段**不适合或需要先补前置条件**，别硬上：

| 场景 | 为什么不适合 | 正确姿势 |
|---|---|---|
| 无测试基建、无法建立门禁的超大遗留单体 | "验证"部件缺失，AI 产出无法收口——门禁绿不了，一切声称都无法核实 | 先补最小门禁（能编译 + 一条冒烟链路），再引入 AI |
| 强实时 / 强算法调优（性能内核、复杂并发调优） | 核心判断依赖人的领域深度，AI 只能辅助分析，提效有限 | 当分析助手用（画调用链、找嫌疑点），别指望端到端交付 |
| 需求方给不出验收标准的纯探索性项目 | 六步法的地基是"可验收"，验收标准缺失时全流程空转 | 先用 AI 快速做原型帮需求方看见想要什么，收敛后再上流程 |
| 涉密等级超出公司允许边界的代码 / 数据 | 红线问题，不是效率问题 | 直接不用（第 14 章） |

**判断标准一句话：能不能建立"可验证的收口"——能，这套方法就适用；不能，先补验证部件，而不是先上 AI。**

---

## 第 2 章 认知对齐：AI 辅助开发是人机协作，你的工具是一个智能体
...
### 2.3 提效的本质：压缩"等待"和"返工"

传统开发中真正写代码的时间占比其实不高，大量时间花在：读懂旧代码、查资料、写样板代码、联调返工、写文档、开对齐会。Claude Code 压缩的正是这些"非创造性时间"——在我们的案例里：读懂存量系统从数周专项变成 2 天走查（工单系统）、批量模块开发从"每个从头做"变成"照配方复制一次编译通过"（商机平台）、日报从每天 15 分钟变 1 分钟（某政）。具体倍数因场景差异很大，所以第 15 章坚持"用自己的基线测"而不引用通用百分比；但方向是确定的：体感提效远超"代码补全工具"。

### 2.4 开发工具的本质：它是一个智能体，而且每个人的都不一样

这是全文档最重要的一次认知对齐：**你的角色不是"AI 的使用者"，是"这个智能体的架构师"。**

先看一张解剖图——你手里那个"工具"，其实长这样：
...
把 Claude Code 这类工具拆开，大致是七个要素（这是我的拆法，不是标准定义；日常交流可以简化成六个部件：**模型 / 工具 / 记忆 / 技能 / 知识库 / 工作流**——"知识库"对应下表的"上下文"，"工作流"对应"指令 + 循环"）。每一块短了，都会以一种很具体的症状表现出来：

| 要素 | 它决定什么 | 这块短了，你会看到 | 怎么补 |
|---|---|---|---|
| **模型** | 判断力的上限 | 评审给出一份"看起来很专业的通过" | 判断类上顶配、机械类降档（9.5） |
| **上下文** | 它基于**什么事实**推理 | 跑偏、幻觉、按过期 spec 改掉你写对的代码 | 一任务一会话、结论落盘、单一事实源（5.3 / 第 7 章） |
| **指令** | 目标与边界 | spec 里没写清的地方，它猜一个实现出去 | 精确化 spec、可验收标准、红线分级（第 4 章 / 10.1） |
| **工具** | 它**能对世界做什么** | 你在人肉搬运报错信息给它看 | MCP 扩边界、Hook 收边界（5.6 / 5.7） |
| **循环** | 自主到什么程度 | **人成了瓶颈**——你在给 AI 当测试员和审查员 | SDD、Agent Teams、无头模式（第 9 章 / 5.11） |
| **记忆** | 边际成本是否**递减** | 每个新项目重新踩一遍同样的坑 | 记忆 → 规则 → 技能 → 钩子（第 10 章） |
| **验证** | 质量地板，以及你敢不敢放手 | 自测全绿、真实文件乱码 | 要证据、真实样本、分层验证（第 13 章） |
...
| 工具 | 默认工具集 | + Playwright MCP、内部平台接口、双层 Hook 护栏（5.6/5.7） |
| 记忆 | 空 | 几个项目的踩坑记录、个人纠偏偏好（第 10 章） |
| 技能 | 无 | 公司 skill 三件套 + superpowers + 自己沉淀的方法论包（4.2/5.5） |
| 知识库 | 无 | CLAUDE.md 项目宪法、决策纪要、公司开发手册（5.1/第 7 章） |
| 工作流 | 一问一答 | 六步法、多角色评审、SDD 流水线（第 3/8 章） |
| 验证 | 信 AI 的一句"完成了" | 门禁全绿 + 真实样本 + 落库核验（第 13 章） |

把两人的七块板逐项打分（0~10），差距一目了然——**唯一持平的就是"模型"那一行**：
...
2. **智能体是养出来的，不是装出来的。** 抄别人的配置只是拿到初始存档：⓪步"继承骨架、改写事实"（4.1）是起点，第 10 章"自我进化"是养成路径。常见疑问"等下一代模型出来这套是不是就不用搞了？"——恰恰相反，**模型越强，另外六块的杠杆越长**：给一个更聪明的工程师看两份互相矛盾的文档，他只会更快、更自信地做错事。
3. **个人独特智能体 + 团队共享层，是双层结构。** CLAUDE.md、门禁、契约进仓库，是团队共享部件；记忆、个人技能、使用习惯，是个人部件。第 12 章的一致性机制管的是共享层，不抹杀个人层。

**这六块板的总和，业内有个专名——"Harness"**（原意是马的挽具：马出的是马力，挽具决定马力有多少能传到车上）。模型就是那匹马，上下文、指令、记忆、知识库、工作流、验证就是你给它配的挽具——**马人人相同，跑得快慢看挽具**。第 10 章"Harness 自我进化"讲的就是让这套挽具越用越强。

**最后是这一节真正的用法——木桶效应：**

> **你的提效上限 = 七块板里最短的那一块。**

模型再强，上下文里躺着两份互相矛盾的文档 → 垃圾进垃圾出（7.4）；上下文干净但没有验证 → 它自信地交付错的东西（商机平台 mock 双重转换教训，12 章）；有验证但没有记忆 → 每个项目从头踩坑（第 10 章）。

**所以别再问"该用哪个模型"了——对着这七块板给自己团队打一遍分，最低的那块就是你下周该补的地方。** 后面 14 章，本质上就是这六块板的展开。

**"提效在部件、不在模型"不是思辨，是做过的实验（商机平台，11.1）**：一名对公司开发环境不熟悉的开发者，继承团队共享部件（公司 skill 三件套、门禁、开发手册），带上自己的个人部件（使用经验与工作流），零手写代码完成了 6.5 万行的 0→1 全栈项目。环境不熟没关系——缺的板，继承过来就补上了。**部件可继承、可迁移、可组合，所以提效可复制、不靠天赋。**

---

## 开工路线图：拿到项目的前三天（全文档主线速查）

全文 17 章讲的所有方法，落到"拿到一个项目"就是下面两条线。先看这一页，再按需跳读。

**新项目（0→1）——商机平台实测路线：**

| 天 | 做什么 | 对应章节 |
|---|---|---|
| Day 0 | 环境与骨架：装好套件，skill 一句话拉起前后端框架，编译/启动/登录跑通 | 3.1 + 《0-1 验证 ①开箱姿势篇》 |
| Day 1 | 需求基线：多角色 PRD 评审 → 决策纪要 → 产品文档终稿（唯一施工基准） | 4.3① / 6.2 / 第 7 章 |
| Day 1 | 项目配置：CLAUDE.md / AGENTS.md / MEMORY.md 三件套，配置本身过一轮评审 | 4.1 / 5.1 |
| Day 2+ | 六步法进入实施：设计 → 计划 → 分块实施（首模块打穿成配方，后续模块照配方复制）→ 验证收口 | 第 4 章 / 9.3 |

**存量项目——工单系统实测路线：**

| 天 | 做什么 | 对应章节 |
|---|---|---|
| Day 1 | 入场三板斧：CLAUDE.md（按本项目栈和安全姿态改写，不照搬）+ AGENTS.md + MEMORY.md | 4.1 / 11.2 |
| Day 2 | 六角色深度业务走查：产出带 file:line 证据的问题文档 | 9.2 / 8.2 |
| Day 3+ | 按走查结论分组并行修复：SDD + 逐任务评审 + 终审门禁 | 9.3 / 第 13 章 |
...
> 🧭 **本部分回答**：一个人怎么把 Claude Code 用对——姿势、纪律、六步法、技巧库。
...
| # | ❌ 常见错误姿势 | ✅ 正确姿势 | 为什么 |
|---|---|---|---|
| 1 | 一句话需求直接开干："帮我加个导出功能" | 先澄清再动手：让 AI 复述需求、列出它的假设和方案，确认后再实施 | 模糊输入 = 大概率返工。第 4 章六步法解决 |
...
| 4 | 项目没有 CLAUDE.md，每次都重新交代背景 | `/init` 生成后**持续维护** CLAUDE.md：技术栈、规范、红线、常用命令 | 这是 AI 的"项目手册"，一次投入长期受益 |
...
```mermaid
flowchart LR
    A["按开源直觉改 env"] --> A1["全部请求 404<br/>应用白屏"]
    B["文档说 starter 配了乐观锁"] --> B1["反编译取证：<br/>只有分页拦截器"]
    C["门禁显示绿"] --> C1["配置评审揪出<br/>2 个假门禁"]
    A1 --> D["🔒 框架当黑盒<br/>以 skill · 门禁 · 字节码为准"]
    B1 --> D
    C1 --> D
...
这是我在五个真实项目（第 11 章）中验证过的标准流程，已写入各项目 CLAUDE.md 成为强制规范。
...
| 查询/解释/生成文档 | 直接问 |
| 涉及鉴权/输入/存储/资金 | 六步 + 强制安全评审（第 14 章） |

**渐进采纳路径**（给"会用但不深"的同事）：第一周只练 3.2 纠错表 + 3.3 需求三要素 + 计划模式——这三样不需要装任何插件；从第一个新功能开始上六步法；多角色评审和 SDD（第 9 章）放到第二阶段。**不要第一天就试图复刻全套——智能体是养出来的（2.4）。**

```mermaid
flowchart TD
    Z["⓪ 开工前：继承并改写项目配置（每个项目只做一次）"] -.-> A
    A["① 需求澄清（brainstorm）"] --> RA{"评审门禁 · 1 轮"}
    RA --> B["② 设计方案（spec 文档）"]
    B --> RB{"多角色评审 · 1~3 轮"}
    RB --> C["③ 实施计划（plan）"]
    C --> RC{"多角色评审 · 1~2 轮 + 人工批准"}
    RC --> D["④ 分块实施（TDD 小步）"]
    D --> RD{"逐块评审 · 每块 1 轮"}
    RD --> E["⑤ 全量终审（整体一致性）"]
    E --> F["⑥ 验证收口（verify + 提交）"]
...
两个项目粗看都是"Spring Boot + Vue3"，实际上 JDK 差了 9 个大版本、数据库一个关系型一个文档型、UI 库和包管理器全不同——**而最后一行是致命的：红线完全相反**。如果把某政的 CLAUDE.md 整个拷进工单系统，你就在一个"绝对不许出现明文密钥"的项目里，白纸黑字写进了另一个项目宽松的凭据管理约定——而且 AI 会**严格遵守**这条规则。配置照搬的代价不是不好用，是**把上一个项目的安全假设，静默地装进了一个假设完全不同的项目**。
...
正确的节奏是：**⓪ 步把骨架和已确认的事实写进去就收口开工，规则条目留白**；之后每遇到一次 AI 在本项目上的典型失误，就沉淀一条精确规则（第 10 章"自我进化"讲的就是这条长大的路径）。**CLAUDE.md 是长出来的，不是抄出来的。**

### 4.2 六步法靠什么跑起来：superpowers 插件

**这套流程不是靠自觉喊口号，而是每一步都有一个 skill 兜底。** 我用的是社区插件 **superpowers**（GitHub：`obra/superpowers-marketplace`，实践时版本 6.1.1），它把"先想清楚再动手"做成了强制流程——你不批准设计，它就不写代码。
...
**关键做法：把它写进 CLAUDE.md 变成项目宪法。** 某政项目的 CLAUDE.md 里这六步是"不可违背"条款——新会话、新同事、新代理进来自动继承，不需要有人在旁边盯。插件只是提供了 skill，**真正让它生效的是你把它固化成项目规则**（见第 10 章"自我进化"四级阶梯）。
...
> 计划阶段评审的价值，某政项目的对照更直观：数据中心需求 **spec 和 plan 双阶段都评**，plan 轮抓到"403 走 HTTP-200+R.code 约定""tenantType 大小写"——**同一批问题若流到编码后才发现，返工成本至少放大十倍**。

**④ 分块实施（TDD 小步）+ 逐块评审** — 每块先写测试再实现，块与块之间可独立验证；每块完成即评审（SDD 模式：实现代理+评审代理接力，见第 9 章），本块 must-fix 清零才进下一块。
...
> 三个好处：**你能真正判断结论对不对**，而不是被术语唬住照单全收；**这四段话可以直接贴进走查文档和汇报材料**，PM、业务方、管理层都看得懂，不用你再翻译一遍；**它讲不清楚的地方，往往就是它自己没想清楚的地方**——这是一个免费的"结论可信度探针"。
...
其中**精确性是含金量最高的一条**：禁止"优化性能""提升体验"这类表述，必须量化成可验收的指标——**"单次查询 ≤10 万条、P95 < 500ms"**。原因在第 4 章开头那句总纲的反面：**spec 里模糊一处，AI 不会回头问你，它会直接猜一个实现出去**——设计文档每模糊一处，就在代码里多一处等着返工的 bug。这条和第 8 章"事实沟通"是同一个信仰：**观点会扯皮，事实不会**。
...
flowchart TD
    ...
    B["📐 设计/计划评审层<br/>拦截：方案漏洞<br/>实例：某政抓到 @EnableAsync 遗漏、ThreadLocal 竞态"]
    C["🧱 逐块评审层<br/>拦截：实现偏差<br/>实例：工单组B 逐任务 must-fix 清零"]
    D["🏁 终审/多角色评审层<br/>拦截：跨块盲区<br/>实例：商机平台抓到方案匹配功能前后端都没做的 P0"]
    E["✅ 流到线上的缺陷 ≈ 0"]
    A --> B --> C --> D --> E
...
按投入产出比排序，前三项是"必装"，后面按需采用。先看全景，再按星级逐个进。

> 📖 **本章读法**：这是一章"技巧库"，定位是**工具书，不必从头读到尾**。赶时间就只看五星三节 + 每节的表格和 📌 一句话，其余各节记住"有这么个东西"即可，用到时再回来查。
...
### 5.1 CLAUDE.md：给 AI 的项目手册（★★★★★）

项目根目录的 CLAUDE.md 每次会话自动加载。我们各项目通用的实践结构：
...
> ⚠️ 关键教训：**不同项目红线可能完全相反**。某政项目（内网私有仓库）经评估沿用配置随仓库管理的约定；工单系统严禁任何明文密钥、必须 AES-GCM 加密入库。CLAUDE.md 按项目写死，AI 就不会拿 A 项目的习惯祸害 B 项目。

### 5.2 计划模式 Plan Mode（★★★★★）🖐️ 建议亲手一试
...
> 📌 **本节一句话**：上下文不是越多越好——一任务一会话勤 `/clear`，密钥类文件用 deny 规则硬拦，别信 `.claudeignore`。

### 5.4 斜杠命令：把重复操作固化（★★★★）🖐️ 建议亲手一试

`.claude/commands/` 下放一个 Markdown 文件，就成了团队共享的斜杠命令。**最值得第一个做的就是日报/周报**——它是全文档里复现门槛最低、受益面最广的一个案例。
...
某政项目里这条命令的真实效果：**每天 15 分钟写日报 → 1 分钟确认**。周报同理，把"今日"改成"本周"即可；月报、迭代总结都是同一个模子。
...
| 一笔账 | 数字 |
|---|---|
| 每天写日报 | 15 分钟 → **1 分钟**（省 14 分钟） |
| 每月累计（约 20 个工作日） | ≈ **5.7 小时，接近一整个工作日** |
| 回本门槛（第 15 章） | 每人每月省出半天即为正收益 |
| 结论 | **仅日报一项已越过回本线，后面所有提效都是净赚** |

正因为**没有争议、没有风险、当天见效**，它适合当团队推广的第一站。

三个设计要点，照抄时别丢：

| 要点 | 为什么 |
|---|---|
| **必须人工确认后才上报** | 上报是对外动作，一旦发出无法撤回。红线：自动审批类动作永远人按最后一个按钮（第 14 章） |
| **命令进 `.claude/commands/` 随仓库分发** | 它就不再是"leon 的个人技巧"，而是全组 clone 即得的团队资产（第 12 章） |
| **把口径固定写进命令里** | 项目名、格式模板、上报接口、扫描范围写死在命令中，AI 每次不用再问、你每次不用再教 |

> 🚀 **进阶**：这条命令还能再往前一步——挂到 CI 上定时跑，扫 git 自动生成周报草稿（见 5.11 无头模式）。但**上报动作仍然建议留人确认**，别让机器替你对外发言。

### 5.5 Skills：可复用的方法论包（★★★★）

比斜杠命令更重的复用单元：一个 SKILL.md 装下完整方法论（何时用、步骤、检查清单、经验教训）。例如我自建的**用户级自定义 skill** `solo-fullstack-project`——存放在 `~/.claude/skills/`，跟人走；**非平台自带，也不属于公司 skill 套件**，是把个人项目经验按 skill 机制沉淀出来的产物。全文 332 行，从项目分期、技术选型到部署清单，新项目直接继承全部历史经验（详见第 10 章"自我进化"）。
...
在工具调用前后插入检查脚本：提交前强制跑 lint、拦截危险命令、格式化落盘文件。与 CLAUDE.md 的区别：**CLAUDE.md 是"请遵守"，Hook 是"违反就拦截"**。团队治理靠 Hook 不靠自觉（第 12 章展开）。
...
> ⚠️ **按角色分档安装**：研发装全量检查，非研发（PM/产品/测试）只装安全扫描那几条。否则一个 PM 改完 PRD 想提交，被 `pre-push` 的编译检查拦下来——他不会修编译，他只会觉得"这套 AI 玩意儿真麻烦"然后再也不用了。第 6 章我们让非开发岗也进仓库用 AI，这个洞就必须堵上。

> 📌 **本节一句话**：CLAUDE.md 是"请遵守"，Hook 是"违反就拦截"——编辑时软提醒、提交推送时硬阻断，非研发只装安全扫描。


### 5.8 并行开发：git worktree + 多会话（★★★☆）
...
> 实例：工单系统组B/组C/组D 三组任务并行推进，互不阻塞（第 11 章）。

### 5.9 无人值守模式（★★★，慎用需授权）

明确授权范围后让 AI 长时间自主推进（自动选型、自动装依赖），保留硬红线（不 push、不删库、不碰生产）。适合：夜间跑长任务、批量迁移、大规模重构。
> 实例：baseProject 授权无人值守后，整个转型实施"设计当天定稿、当天交付"。**前提是红线写清楚 + 事后全量验证**，不是裸奔。商机平台的多路后台子代理并行打磨是它的近期变体：授权范围收窄到"各自模块的文件"，门禁中心化兜底（见 9.3）。

### 5.10 Token 经济学：省钱与提质的双赢技巧（★★★★）
...
| deny 规则排除 dist/lock/大 JSON（见 5.3） | 无关大文件不进上下文 | 这些文件本来就不该参与推理 |
...
- **需求三要素一次给全**——挤牙膏式往返，每一轮都在为全部历史重复付费，而且模糊期的错误尝试会永久留在上下文里；
- **给参照文件路径**，省掉一次全库搜索；
- **明确"只改哪几个文件"**，防止大范围读写。

**⑤ 止损比修补便宜**——三种跑偏程度，对应三个动作：

| 情形 | 动作 | 为什么便宜 |
|---|---|---|
| 刚开始跑偏 | 立刻按 Esc 打断 | 错误产出、纠正、重来三份内容都计费，还会污染后续推理 |
| 文件改砸了 | `/rewind` 检查点一键回退（Claude Code 内置，比 git 回退更轻） | 只回文件状态，会话不用重建 |
| 错得离谱 | git 回退 + `/clear` 带一句教训重开 | 同会话"对话式修补"更贵且质量更低——AI 始终带着失败历史的惯性 |

**⑥ 机制层**：机械性任务（批量格式化、i18n 补键）可显式用小模型，核心设计/评审用大模型——**省要省得明白，不接受静默降级**（与第 9 章"派 agent 显式指定模型"同一原则的两面）；同一任务集中连续推进可命中提示缓存，比零碎聊一天便宜。

> 📌 **本节一句话**：省 token 和提质量是同一件事——让每个 token 都只为"新信息"付费。

### 5.11 CI/CD 无头集成：人不在场也在提效（★★★★）
...
| **夜间定时走查** | 定时任务每晚跑一轮安全走查/依赖审计，报告落文件 | 早上上班看报告，第 14 章"AI 安全走查入流程"零人力成本落地 |
...
- **权限最小化**：CI 里的 AI 只读 + 产出报告，不自动改码、不自动合并；
- **产出物统一落到固定路径**，供人复核；
- **从零风险场景起步**（评审建议类），跑稳了再扩权；
- **计费两条路**：`claude setup-token` 生成长期令牌复用现有订阅额度（与交互用量共享限额），或走 API Key 按量计费——试点期实测用量再定。

这条线的 ROI 很直观——**同一套工具链，产出时间从 8 小时/天变成 24 小时/天**。

---

# Part C 岗位视角与全流程协同

> 🧭 **本部分回答**：从"一个 AI 助手"到"一支 AI 团队"——每个岗位怎么切入、多代理怎么编排、环境怎么越用越强。

## 第 6 章 岗位提效地图（7+1 个岗位）

> 本章目的：让每个岗位的人都找到"跟我有关"的第一个切入点。**按岗位对号入座读：只看自己那一节 + 6.8，一两分钟就够**——每节就是一张"高频场景 → 怎么用"的表，末尾一行提效亮点，不必通读八个岗位。
>
> 📖 **读法说明**：表中场景都是**通用能力**，出现的项目名（商机平台 / 工单系统 / 某政）只是实测出处和证据——**方法可直接移植，把对象换成你自己的项目即可**。
...
| 周报/日报自动化 | `/daily-report` 类命令扫 git 记录自动生成（某政项目实例） |
...
**提效亮点**：
- 大体量 PRD 的收口流水线：多角色独立评审 → 交叉核对 → 拍板写成决策纪要 → 终稿即唯一施工基准（实例：商机平台 42 个 FEAT 四角色交叉评审，产出 D1~D9 决策纪要与 v2.0 终稿）
- 关键纠错一例：登录方案差点按"自建账密"写错，调研证实框架默认就是企业 SSO——**评审阶段一次纠错，省掉整个鉴权模块的返工**
- 次选技巧：UI 选型"用看的不用猜的"，让 AI 生成多套自包含 HTML 样张供干系人直接挑
...
| 技术栈迁移 | 整栈迁移三件套：设计文档先行、双端并行、API 契约 100% 对齐（实例：某政项目 NestJS→Spring Boot 整体迁移，前端零改动） |
...
| 设计稿/原型高保真还原 | 通用四步：先并行审计缺口 → 夯地基（契约 + 共享组件 + 样板页）→ 多路并行镜像样板 → 中心化门禁收口（实例：商机平台 30 页原型全量还原）；教训：**不盯原型比对，AI 默认容易退化成"表格+表单"** |
| Mock 管理与切真 | mock 按"接口白名单 / 数据保留 / 页面碎片清剿"三层有序退场（实例：商机平台 20 个模块）；教训：**mock 有第三层——页面级 mock 没有 api 文件，端点盘点会漏** |
...
**提效亮点**：还原和退场都有配方——还原走"审计缺口 → 夯地基 → 并行镜像 → 门禁收口"四步，mock 走三层有序退场；唯一要人盯住的是原型比对，不盯 AI 就退化成"表格+表单"。
...
| 日志与可观测 | 高写日志表设计模式：BIGINT 自增主键+异步落盘+ThreadLocal 快照要在异步跳变前取（某政项目沉淀进 skill 的教训） |

**提效亮点**：后端提效就两个词——"配方"和"取证"：首模块竖切打穿沉淀成配方，后续模块照抄（商机平台第二个模块一次编译通过）；拿不准框架行为不靠猜，反编译字节码或最小复现一锤定音。
...
| 穷举校验 | 关键状态/规则组合从"抽样测试"升级为"全矩阵穷举测试"——AI 写用例的成本足够低，穷举变得可行（实例：工单系统状态机 8×8 全组合，人工不可能做到的覆盖密度） |
...
| 多角色深度走查 | 多视角走查能挖出长期存在、人工一直没发现的漏洞（实例：工单系统六角色走查一次挖出 3 个 P0——**JWT 弱默认密钥可离线伪造 admin 令牌**、**Hermes baseUrl 用户可控→SSRF+API Key 外泄**、**幂等 marker 跨工单污染**） |
...
**提效亮点**：安全从"上线前扫一遍"变成全程闭环——多角色走查能挖出长期存在、人工一直没发现的漏洞，且每个修复都要让攻击用例由红转绿才算修完。

### 6.8 全栈工程师 / 独立交付者

全栈是 AI 辅助开发的最大受益者。全栈的老大难在"覆盖面"：前端、后端、数据库、安全、测试样样都要沾，一个人的知识面和精力横向铺不过来——现在缺哪块，AI 就补哪块，覆盖面不再是瓶颈。第 11 章的五个真实项目就是证据。核心心法：
...
### 7.2 解法：文档链 = 单一事实源（Single Source of Truth）
...
> 实测效果：商机平台的"决策纪要 = 唯一基准"是这套机制在多代理场景的压力测试——8 路子代理并行铺开 30 个页面，全靠决策纪要锁口径才没有漂移；某政项目 spec+plan 双阶段评审**每一轮都抓到会导致返工的真问题**；工单系统每个任务组固定"知会 Architect 契约变化"收尾，跨端契约从未失配。

### 7.4 需求变更与逻辑漂移：必须全局统一，不能只改一处
...
> 这是从"一个 AI 助手"升级到"一支 AI 团队"的分水岭，也是本文档的进阶重点。

### 9.1 Subagent（子代理）：任务级分身
...
> ⚠️ 实践约定（工单系统沉淀）：派子代理时**必须显式指定模型**——子代理不继承主会话的模型，不指定就是静默降级。怎么按角色分档配、省在哪不能省在哪，见 9.5。

### 9.2 多角色评审：子代理的最高性价比用法 🖐️ 建议亲手一试
...
真实战绩回顾：
- 商机平台五角色评审 → 联调前抓到一个 P0："方案匹配"功能前后端都没做——后端没写接口，前端靠写死在页面里的假数据撑着、连独立的接口文件都没有，所以按接口文件清点出的 68 个端点里根本没有它，**工具扫不出来，只有评审能抓到**
- 工单系统六角色业务走查 → 挖出 3 个 P0 安全漏洞
- 工单系统状态机简化 → 五角色评审一致放行才实施，删掉 8 个死状态零回归
- 某政项目 spec+plan 双阶段多角色评审 → 每轮抓到真 blocker

**为什么有效**：单一视角有盲区，角色提示词强制切换关注面；多个独立上下文互不干扰，相当于真开了一场评审会，但成本是几分钟。

### 9.3 SDD：子代理驱动开发（Subagent-Driven Development）
...
4. AGENTS.md 定义角色分工表（某政/工单系统均建立 6 角色分工），角色职责、交接物、通知义务写清楚

### 9.5 给每个代理配对模型：兼顾 token 与效果
...
参考量级（官方 API 价目，每百万 token 输入/输出，**截至 2026-07**；Sonnet 5 至 2026-08-31 有 $2/$10 的首发优惠价）：**Fable 5 $10/$50 · Opus 5.8 $5/$25 · Sonnet 5 $3/$15 · Haiku 5.5 $1/$5**。也就是说**同样的输出量，Haiku 比 Opus 便宜约 5 倍、比 Fable 便宜 10 倍**。订阅制下你付的不是账单而是额度，但额度消耗的速率差是同一个比例——**六角色评审全上顶配和"两个判断角色上顶配、四个检索角色用轻量档"，跑一天的额度差得非常明显**（计费两条路见 5.11）。

**第二个旋钮：推理力度（effort）。** 除了换模型，还能调同一个模型想多深（`low` / `medium` / `high` / `xhigh` / `max`）。机械型子代理开 `low` 就够——**它少想、少绕、少输出，省的是纯浪费的那部分**；终审和难题留给 `high` 以上。

**配在哪：写进 `.claude/agents/*.md` 的 frontmatter**（模型和推理力度都可以固定在角色定义里），随仓库进版本库——**全团队派出的"安全评审员"就都是同一个档位的安全评审员**，不靠每个人记得手动指定（第 12 章"一致性靠机制不靠自觉"）。
...
> 大多数人用 AI 工具是"每次从零开始"；高手的环境是**用得越久越强**。这就是自我进化机制——把每一次纠偏、每一个教训固化为环境资产。本章全部机制均已在多个真实项目中实际运行。
...
| ③ 技能 | `~/.claude/skills/*/SKILL.md` | 可复用方法论——跨项目继承 | solo-fullstack-project（自建用户级 skill，332 行，含 Lessons Learned 演进章节） |
...
> ℹ️ **先说归属，避免误解**：本节这套 `memory/` 目录结构与"会话自动加载"机制由**开源插件 ECC** 提供，**不是 Claude Code 原生功能**（原生记忆只有 CLAUDE.md）——归属细节与治理答疑见第 17 章。本节讲的是我们怎么用它。
...
### 10.3 Skill 演进：方法论的复利

自建用户级 skill `solo-fullstack-project`（见 5.5）的演进规则（写在 skill 内部）：
...
**还有一层：组织级进化——用实战反哺工具团队。** 商机平台 0→1 实验的副产品，是一份给公司套件团队的实战反馈清单（SSO 联调卡点、插件注册表 bug、starter 文档与字节码不符、脚手架残留触发假门禁）。这个环是：使用者实战暴露 skill 待加强点 → 反馈工具团队 → skill 升级 → 全员受益。**个人进化让你自己的智能体变强，这个环让所有人的"共享部件"变强（2.4 推论三）。**

> 换电脑实证：Mac→Windows 迁移时，skills + memory 全部随身迁移，全部项目的经验资产零丢失。**员工的隐性经验变成了可迁移、可继承、可审计的显性资产**——这一点的组织意义不亚于提效本身。

### 10.6 环境即资产：换设备只需带走一个文件夹
...
**真实案例就是本文档自己**：Mac→Windows 换机后，从旧 `.claude` 文件夹里完整恢复了某政/baseProject/web-tool 三个项目的全部项目记忆、踩坑教训和 skill——本文档第 11 章的案例数据全部来自这次恢复。
...
> 🧭 **本部分回答**：以上方法真做得成吗？两个最新项目从头到尾的证据。

## 第 11 章 双主案例：商机平台（0→1）与工单系统（存量）

> 两个最新项目各讲透一条线：**商机平台**回答"拿到一份 PRD，AI 能从零做到联调吗"；**工单系统**回答"接手一个跑了很久的系统，AI 能干什么"。更早的三个项目压缩为速览（11.3）。

### 11.1 主案例一：销售商机互助平台（0→1 全链路 · 公司框架 · 2026-07）
...
**核心经验**：
- **黑盒纪律是 0→1 成败的分水岭**（3.4）：早期返工的约一半来自 AI 用开源直觉碰框架禁区；纪律立住之后返工断崖式下降
- **配方化 = 边际成本断崖**：requirement 首模块慢慢打穿，沉淀成 12 类文件的竖切配方；opportunity 照配方复制**一次编译通过**——这就是"为什么越用越快"的直观答案
- **决策纪要 = 并行不漂移的根**：8 路子代理并行铺开全靠它锁口径（第 7 章）
- **评审抓到工具盘点不出的洞**：五角色评审揪出"方案匹配"功能前后端都没做的 P0——假数据写死在页面里、没有接口文件，端点盘点根本看不见它，只有评审能抓到
- **"200 不算数，落库的行才算数"**：单测全绿库仍可能是错的，JDBC 核库与门禁绿同权重（第 13 章）
- 全程实战记录（含截图）见飞书《0-1项目验证》；方法论已沉淀为《0-1 项目提效验证》系列 8 篇，照着做即可复用

### 11.2 主案例二：工单系统（存量项目提效——对多数团队最有参考价值）
...
| 天 | 动作 | 产出 |
|---|---|---|
| Day 1 | **入场三板斧**：建 CLAUDE.md（按本项目栈和安全姿态改写，与某政完全不同）+ AGENTS.md（6 角色）+ MEMORY.md，外加敏感/大文件排除规则 | AI 完成项目认知；顺带发现 4 个存量问题 |
...
- 诚实汇报文化：文档里明确记着"JWT 弱默认密钥仍未修"、"集成测试待 Docker 复跑"——**AI 时代的工程诚信是刻意设计出来的**（第 13 章）

### 11.3 更多案例速览（早期项目）

| 项目 | 一句话战绩 | 独有看点 |
|---|---|---|
| **某政项目**（已上线） | 一人交付 Web+H5+Java 三端；NestJS→Spring Boot 整栈迁移，双端并行、API 100% 一致、前端零改动；spec+plan 双阶段评审抓到 @EnableAsync 遗漏、ThreadLocal 竞态等真 blocker | 技术栈整体迁移方法论；安全加固冲刺一轮清零四类问题（35 处 any 消除、37 个测试补齐）；/daily-report 日报自动化的发源地 |
...
| | 商机平台 | 工单系统 | 某政 | baseProject | web-tool |
|---|---|---|---|---|---|
...
> 🧭 **本部分回答**：十个人一起用怎么不翻车——一致性、质量、安全三道防线。

## 第 12 章 团队一致性机制

> **一致性靠机制，不靠自觉。** 十个人各自用 AI，没有机制就是十种风格、十套流程、十个风险面。

### 12.1 三层共享配置（随仓库分发，clone 即继承）
...
**进阶：团队共享记忆也能进仓库（`@import` 方案）**

先纠正一个容易想当然的做法：在 `.claude/` 下自建一个 `memory/` 目录，Claude **不会**自动加载——`.claude/` 只有 `settings.json` / `commands/` / `agents/` / `skills/` 这几个约定位置有效力，原生默认加载进上下文的只有 CLAUDE.md。想让项目记忆随仓库分发并自动加载，用 CLAUDE.md 的 **`@import` 语法**（原生功能）：

```
项目仓库/
├── CLAUDE.md                 ← 加一行：@.claude/memory/MEMORY.md
└── .claude/memory/
    ├── MEMORY.md             ← 索引：一行一条，指向具体文件
    ├── pitfall-xxx.md        ← 具体记忆，AI 按索引按需读取
    └── decision-xxx.md
```

CLAUDE.md 加载时会把 `@` 引用的文件一并注入——等于用原生机制复刻了"索引常驻、正文按需"的记忆模式，且随 git 分发、全组 clone 即得。两个注意点：import 有嵌套深度上限（5 层），别套娃；索引是每个会话的固定 token 开销，MEMORY.md 只放一行式索引，正文留在子文件。

**定位**：这不是把个人记忆搬家，而是 10.5"个人资产 → 团队资产"升格通道的落地机制——个人踩坑记忆留在插件记忆层（`~/.claude/`，跟人走）；经评审值得全组共享的，走 PR 进仓库内 `.claude/memory/`（跟仓库走）。两层各司其职，与 10.6 的"个人资产在 `~/.claude/`、团队资产在仓库"一一对应。

### 12.2 CLAUDE.md 团队维护规约

1. CLAUDE.md 变更**走 PR 评审**，和代码同等待遇（它就是"给 AI 的代码"）
2. 每个"AI 干了蠢事"的案例 → 反思是否该补一条规则
3. 定期瘦身：过时规则删除，太长的下沉到子文档
4. **各项目独立成文，禁止跨项目复制粘贴**（真实教训：某政项目与工单系统的凭据管理红线完全相反，抄错红线就是事故）

### 12.3 一致性检查的自动化（Hook + CI）

每一层都要能回答"装在哪、怎么生效"，否则就只是口号：

| 层 | 机制 | 装在哪、怎么生效 | 例子 |
|---|---|---|---|
| 会话内（AI 侧，软提醒） | Claude Code Hook | `.claude/settings.json` 的 hooks 配置**进版本库**，clone 即生效：AI 每次 Edit/Write 后自动跑检查脚本，结果注入会话提醒 AI 当场修 | 改完文件自动跑 lint / 敏感信息扫描，有问题立刻提示，不阻断 |
| 提交时（Git 侧，硬阻断） | git pre-commit 钩子 | 仓库自带安装脚本（如 `scripts/install-hooks`），成员**执行一次**注册进本地 `.git/hooks`；漏装也不怕，后面还有 CI 兜底 | 提交时跑密钥扫描 + 格式化 + 编译，失败直接拒绝 commit（与上一行构成 5.7 的双层防线） |
| 仓库级 | CI 门禁 + 分支保护 | 流水线配置文件进仓库；分支保护设"检查全绿才允许合并"——**服务端强制，本地怎么绕都没用** | 测试全绿 + 类型零错才能合并（web-tool 每个 PR 实测标准） |
| 评审级 | CODEOWNERS + 评审规则 | CODEOWNERS 文件进仓库：指定路径的改动自动拉指定评审人，代码平台强制执行 | 涉敏目录改动自动指派安全评审；契约文件改动自动知会架构师 |

另外注意：本地钩子要**按角色分档安装**（研发全量、非研发只装安全扫描），原因和做法见 5.7 末尾的说明——**门禁是用来拦风险的，不是用来拦人的**。
...
### 13.2 TDD 是 AI 协作的最佳拍档

TDD（Test-Driven Development，测试驱动开发）核心就一条规则：**先写测试，后写实现**——功能还没做时先写一批"必然失败"的测试用例（它们描述的是做完之后应该有的行为，如"输入订单号 X 应返回状态 Y"），然后写实现代码，直到测试全部转绿。绿了 = 行为达标。

用在 AI 协作上，它解决的是**人的审查带宽跟不上 AI 的产出速度**：

1. AI 先写失败测试——每条测试就是一句可执行的需求断言；
2. **人只审这十来条测试**：对照需求看"输入→输出"对不对，一眼能判——这叫审"行为"而不是审"写法"；
3. 测试确认无误后，AI 放手实现到全绿——写歪了测试当场变红抓住，人不必逐行看 500 行实现。

**测试即规格**：审完这 10 个测试用例，等于审完了 AI 对需求的全部理解——比审 500 行实现快一个数量级。
> **实例（工单系统状态机改造）**：工单有 8 个状态，任意两个状态之间"允许 / 禁止流转"共 8×8 = 64 种组合。传统抽样测试只挑几条主流程（如 新建→处理中→已解决），非法流转是否被拦全凭信仰。TDD 的做法：让 AI 先把 **64 格全部写成测试**，每格一条行为断言——比如 PM 拍板"RESOLVED 是唯一终态"后，RESOLVED 那一行的 7 条外流全部断言"必须拒绝"。人审的就是这张 64 格的对错表，对照业务规则逐格打勾，几分钟的事；然后 AI 改实现直到 64 条全绿，最终整个改造 **322 个单测 0 失败**。这种覆盖密度人工手写不现实，AI 写只是多花几秒——**TDD + AI，穷举第一次变得便宜**。
...
AI 干完活天然倾向报喜——"已完成✅""基本都好了"。真正危险的不是它做错的部分，而是**没做的部分被一句"差不多"盖过去**：你以为收工了，其实集成测试没跑、代码没提交，雷埋进了下一个环节。

指望 AI"自觉诚实"不可靠；可靠的做法是把诚实做成**格式要求**——在 CLAUDE.md 里固定汇报模板为三栏：

| 栏 | 要求 | 堵住什么 |
|---|---|---|
| ✅ 已完成 | 每条必须附证据（测试通过数、门禁输出） | 空口宣称"做完了" |
| ❌ 未完成 | 写明原因 | 没做的事被悄悄略过 |
| ⚠️ 风险与待办 | 下一步谁该做什么 | 交接断链 |

模板一固定，"差不多都好了"这句话**没有格子可以填**——含糊在格式上就不成立。

> 实例（工单系统一次修复的收工汇报）："✅ 本地门禁全绿：单元测试 24/24、前端测试 276/276 通过；⚠️ 待办：① Docker 全量集成测试尚未复跑 ② 代码未提交（等人工确认） ③ 需知会 Architect。"——注意：这是一次**一切顺利**的汇报，AI 仍被格式逼着显式列出三件没做完的事。这就是"设计出来的诚实"。
...
### 14.2 权限分级（Claude Code 内置机制）

| 模式 | 行为 | 适用 |
|---|---|---|
| 默认（逐项确认） | 每个写操作/命令需人工批准 | 新手、敏感项目 |
| 项目级白名单 | settings.json 允许常用安全命令 | 团队标准配置（进版本库统一） |
| 无人值守 | 大范围自主执行 | 显式授权+红线保留+事后全量验证（见 5.9） |

**不想每次都点确认，怎么配？** 按推荐顺序三档：

1. **精确放行（推荐）**：把确认过安全的操作写进 `permissions.allow` 白名单，清单内免确认、清单外照常弹窗。配置文件三个位置任选：`~/.claude/settings.json`（个人全局）、项目 `.claude/settings.json`（团队共享，进版本库）、项目 `.claude/settings.local.json`（个人偏好，不进版本库）：
...
2. **调宽默认模式**：配置里加 `"permissions": { "defaultMode": "acceptEdits" }`——文件编辑自动接受，命令仍需确认；会话内按 `Shift+Tab` 也能随时切换这一档。
3. **全跳过（慎用）**：命令行 `claude --dangerously-skip-permissions`，等价的配置写法是 `"defaultMode": "bypassPermissions"`——所有确认全部跳过。**只在隔离、可丢弃的环境用**（容器、试验仓库、git worktree 副本），连生产凭据的机器和正式仓库不要开。

规则优先级 **deny > allow**：deny 清单在任何模式下都拦得住。所以正确姿势是先把 14.3 的红线写进 deny，再放宽确认——**放开的是"确认动作"，不是"红线"**。

### 14.3 不可自动执行清单（我们各项目的公约数）
...
担心 AI 引入安全风险是对的，但账要算全：工单系统那 3 个**长期存在、一直没被发现**的存量高危漏洞，就是 AI 一轮六角色走查挖出来的（见 11.2）。配合红线机制，AI 对安全是显著净增益。建议把"AI 安全走查"纳入固定研发流程（每次大版本前跑一轮六角色走查）。

---

# Part F 量化与落地

> 🧭 **本部分回答**：怎么度量、怎么算账、怎么在团队里一步步推开。

## 第 15 章 提效度量体系与成本测算

> 📌 **本章一句话**：别用感觉，用基线对比；每月每人省半天以上即回本——日报自动化一项就够了。

### 15.1 度量原则：别用感觉，用基线对比

试点前先测 2~4 周基线，再对比。推荐指标对齐 **DORA 四指标**——业界研发效能度量的事实标准，指 Google 旗下 DORA（DevOps Research and Assessment）团队定义的四个核心指标：**部署频率、变更前置时间（代码提交到上线的耗时）、变更失败率、故障恢复时长（MTTR）**，前两个量速度、后两个量质量，每年发布行业基准线可直接对标：
...
> 🔧 **采集自动化**：Claude Code 原生支持 OpenTelemetry 指标导出——使用量、成本、会话数可接入任何兼容 OpenTelemetry 的监控平台（Prometheus/Grafana 等通用开源方案即可搭建，若已有现成监控体系可直接复用），使用侧数据零人工统计；DORA 指标从 CI/CD 与 git 记录自动计算。**度量本身不允许增加开发者负担**，否则数据一定失真。
...
经验法则：一个研发人员月成本数万元，订阅费约为其 1%~3%。**只要每月为每人节省半天以上，即为正收益**——从我们的案例看（日报自动化每天节省 14 分钟一项就够了），这个门槛非常容易越过。

| **1%~3%** | **半天 / 月** | **14 分钟 / 天** |
|:---:|:---:|:---:|
| 订阅费占人力成本比例 | 回本门槛：每人每月省出这些即回本 | 仅日报自动化一项的节省<br/>——单项已越过回本线 |

真实的争议从来不是"值不值"，而是"如何让不熟练的人也达到熟练者的收益水平"——这正是本文档和第 16 章路线图要解决的。
...
### 15.4 个人 AI 协作水平怎么评：业界成熟做法

15.1~15.3 度量的都是项目和团队的**结果**，还差一维：每个人用得怎么样。团队规模化推广后这个问题迟早要回答——不评价，帮扶和授权没有依据；评价错了（比使用时长、比 AI 代码占比），立刻催生"为用而用"的表演。业界工程效能领域（DORA / SPACE 框架及各大厂效能团队的公开实践）在这件事上已经趋同为三条共识：

1. **评"姿势"不评"用量"**：用量指标全部可刷（见 15.1 反指标警告）；姿势——会不会配置、走不走流程、有没有验证收口——刷不出来；
2. **被动采集、零填报负担**：全部用现有系统里已经产生的数据推断，不考试、不填报、不打扰——拿到的才是真实行为数据；

**数据从哪来——三个现成数据源，零新增负担**：

| 数据源 | 怎么接 | 能看出什么 |
|---|---|---|
| AI 编码工具遥测 | Claude Code 原生支持 OpenTelemetry 导出，接入任一兼容的监控平台即可（见 15.1 采集自动化） | 活跃度（会话数/活跃天数）、使用深度（子代理、斜杠命令等事件）、AI 产出采纳率（编辑接受/拒绝比） |
| git 仓库 | 已有，加一个统计脚本 | 资产贡献（CLAUDE.md / `.claude/` 命令与 skill 的提交及被他人复用）、"先测试后实现"的提交模式、提交规范化程度 |
| CI / 门禁 | 已有 | 门禁一次通过率、返工率、变更失败率 |

**能力阶梯怎么分——常见的四级达标制**（判定信号全部来自上表，无需本人参与；阈值用试点队伍的真实分布校准）：

| 级 | 含义 | 被动判定信号 |
|---|---|---|
| L1 会用 | 真实在用 | 周活跃会话达标；所在仓库有其维护的 CLAUDE.md |
| L2 会走流程 | 按方法论干活 | 提交中出现 spec/plan 文档与先测试后实现的模式；门禁一次通过率达标 |
| L3 会编排 | 用得深 | 遥测中出现多代理/后台任务事件；仓库有其提交的 deny 清单 / Hook 配置 |
| L4 会传承 | 反哺团队 | 其贡献的规则/命令/skill 被 ≥2 个其他成员或项目复用 |

**业界落地时的通行配套原则——透明采集**：被动采集的"无感知"指的是**零填报负担**，采集哪些数据、达标怎么计算，通常都有明确标准
## 第 16 章 落地路线图

> 📌 **本章一句话**：业界优秀公司推广 AI 研发能力的通行节奏是"试点→规范化→规模化"、每阶段有门禁；可复制的是姿势，不是天赋。本章讲的是行业通行做法与我们项目里验证过的经验，供各团队按需取用。

### 16.1 三阶段推进（业界通行节奏）

业界推广 AI 编码能力的公开实践，基本都收敛到同一个三阶段模式：
...
flowchart LR
    ...
    P2 --> G2{门禁:80%成员<br/>达L2基本功水平} --> P3
```

**阶段 1 试点**：挑选标准——自愿+骨干+真实项目（不要拿玩具项目试点，数据没说服力）。种子队员即未来各团队的"AI 教练"。

**阶段 2 规范化**：把试点沉淀为《团队 AI 协作规范 v1》（CLAUDE.md 模板、红线清单、门禁标准、本文档附录的命令库）；培训常用"1 天课程 + 2 周结对"模式；出口标准可直接取用**附录 D 基本功清单**（10 项实操，结对演示 8/10 通过）。

**阶段 3 规模化**：新项目默认带 AI 配置脚手架；把"AI 安全走查"纳入版本发布流程；月度度量汇报驱动持续改进。

### 16.2 角色分工

业界落地案例中常见的最小角色配置（一般不新增编制，多为兼任）：

| 角色 | 职责 |
|---|---|
| 高层发起人 | 定调、给试点授权、看月度数据 |
...
| 阻力 | 心声 | 应对 |
|---|---|---|
| "AI 写的代码我不放心" | 质量担忧 | 看门禁机制（第 13 章）：AI 代码过的关卡比人写的更多；工单系统安全走查挖出存量漏洞（11.2）是最直接的实证 |
| "我自己写更快" | 熟练者的真实感受（前两周） | 承认学习曲线；结对让他看到熟练者的真实速度；从"读代码/写测试/查资料"等无争议场景切入 |
| "那是你厉害，一般人用不起来" | 能力怀疑 | 商机平台实验专门验证了这一点：一名不熟悉公司环境的开发者，凭公司 skill + 正确姿势零手写完成 0→1 全栈项目（11.1）——**可复制的是姿势，不是天赋** |
| "会不会替代我" | 职业焦虑 | 对照人机分工（2.2 节）：AI 接走的是执行，决策、审查、验收的价值反而更高了 |
| "又来一个新工具，过阵子就凉" | 变革疲劳 | 试点数据说话；把工具收益直接兑现到个人（少加班、少写重复代码） |
| "太贵了" | 成本顾虑 | 15.2 成本模型：订阅费 ≈ 人力成本 1~3%，半天/月即回本 |

### 16.4 第一周就能做的五件事（行动清单）

- [ ] 每人：在自己项目跑 `/init` 生成 CLAUDE.md，补齐技术栈/规范/红线三节
- [ ] 每人：用"计划模式"完成一个真实任务，体验批准制
- [ ] 团队：建 `.claude/commands/` 放入第一个共享命令——**建议就从 `/daily-report` 开始**（5.4）：零风险、当天见效、每人每月省近一个工作日，单这一项就越过回本线
- [ ] 团队：定出本团队"不可自动执行清单"（抄 14.3 改）
- [ ] 有意试点的团队：选定试点项目和种子队员，先测基线数据

---

## 第 17 章 边界与答疑：原生 / 插件 / 自建内容

> 本章服务于关心插件归属、治理与审计边界的同事。核心只有一条主线：**把三样东西分清楚——① 哪些是Claude Code 平台原生自带的；② 安装的开源 harness 插件（我们实践中用的是 ECC）提供的规范；③ 我们自己填充的内容**。避免把插件能力误当平台功能，也避免把"内容"和"规范"混为一谈。

### 17.1 一张表分清"原生 / 插件 / 自建内容"
...
**一句话**：第 10 章"四级固化阶梯"里，②规则、④钩子的**载体**来自平台/插件；①记忆的**结构化规范来自 ECC 插件**、**内容才是我们产出的**；③技能的自进化约定是我们写在自己 skill 里的。**记忆系统之所以能进 Git、可评审、可迁移，不是因为我们另造了一套私有脚本，而是因为 ECC 选择用纯文本 Markdown 做载体——规范随插件升级，内容归我们所有。**

### 17.2 记忆系统：ECC 插件提供的规范，既非原生、也非自造

`memory/`+`MEMORY.md` **不是 Claude Code 原生记忆**——平台原生的记忆只有 `CLAUDE.md`（项目级 + 用户级，配合 `@import`、`#` 快捷追加、`/memory` 命令），没有分类、没有索引。**它也不是我们手写的一套约定**——这套"一事一文件 + 四分类 + 索引 + 会话自动加载"的规范由**开源插件 ECC** 在原生 `CLAUDE.md` 之上提供（ECC 的 `knowledge-ops` skill 里明确定义了这个"记忆层"）。**我们贡献的是内容，不是规范。**

这一点对治理很关键，两个常见疑问就此有了明确答案：**谁维护？**——开源社区随插件版本维护，不是团队内部的一次性脚本；**能不能审计？**——纯文本 Markdown 进 Git，每一条记忆的增删改都可 diff、可追溯。
...
| 加工 | 一条一个确定事实，人可读 | AI 自动把会话记录压缩成结构化条目 |
| 存储 | 纯 Markdown，进 Git，可 diff/审计 | SQLite + 向量库，后台进程，检索接口查询 |
| 治理 | 少而精，错的删掉 | 多而全，"记录一切" |

概括：**claude-mem 是"录下全部再检索"（捕获导向、黑盒 DB）；ECC 记忆层是"只沉淀少数确定的真理"（策展导向、白盒可审计）。** 前者省人力，后者上下文开销小、适合团队沉淀可评审的经验资产。两者都是开源插件，不冲突、可并存。

### 17.3 记忆由谁写入？——AI 执行、人把关，非后台守护进程
...
两个触发器（见 10.4）：① AI 在里程碑节点主动提议"要不要沉淀"；② 人当场纠偏时说一句"记住这一点"→ 写入 feedback 记忆。**注意：ECC 虽是插件，但它不在后台默默改你的记忆文件——每一条写入都由当次会话触发、可见、可确认。**

### 17.4 "Skill 自进化"= 特定 Skill + 永远人工确认

两个必须澄清的边界：

1. **是特定 Skill，不是全体**。自进化能力是**写在 `solo-fullstack-project`（自建用户级 skill，见 5.5）这一个 skill 内部的 `Skill Evolution` 小节**，不是平台对所有 skill 生效的功能。没写这段的 skill 不会进化。
2. **永远人工确认，禁止 AI 自动改写**（10.1 铁律）。AI 只在触发点**提议**"本次经验是否更新到 skill？"，改不改、怎么改由人拍板。

**没有任何 skill 会在你不知情时被自动改写；"自进化"指的是"AI 提醒你该沉淀了"，落笔永远是人。**

### 17.5 三重隔离：防串扰、防泄露、防错误沉淀
...
3. **每个 teammate 显式配模型 + effort**——不指定就静默降级；分档原则与配置位置见 9.5，此处不重复
...
**一句话**：Agent Team 不是"一键开团队"的按钮，而是 **"AGENTS.md 角色表 + 每人显式配模型 + 契约文档 + Lead 集成门禁"这套编排纪律**，底层用平台的多代理 + 代理间通信能力。纪律到位才提效，缺一环就变返工（反模式见 9.6）。

---

# 金句速记墙（贴工位版）

> 全文 17 章浓缩成 11 句。记不住方法论没关系，先把这面墙贴起来。

| # | 金句 | 出处 |
|:---:|---|:---:|
| 1 | **你用的不是工具，是智能体——只有模型人人相同** | 2.4 |
| 2 | **模型越强，另外六块部件的杠杆越长** | 2.4 |
| 3 | **AI 提供马力，skill 与门禁提供刹车和护栏，人掌方向盘** | 1.3 |
| 4 | **文档和直觉都可能骗你，字节码不会** | 3.4 |
| 5 | **假绿比红更危险，因为它让你停止怀疑** | 3.4 / 9.5 |
| 6 | **CLAUDE.md 是长出来的，不是抄出来的** | 4.1 |
| 7 | **观点会扯皮，事实不会** | 8.1 |
| 8 | **200 不算数，落库的行才算数** | 13.4 |
| 9 | **红线之所以是红线，是因为它稀少** | 10.1 |
| 10 | **可复制的是姿势，不是天赋** | 16.3 |
| 11 | **模型是大家的，智能体是自己的** | 全文主线 |
...
| settings.json `permissions.deny` | 排除敏感/大文件读取（官方机制，勿用 `.claudeignore`，见 5.3） |
...
> **全文完。** 商机平台 0→1 全程实战记录（含截图）见飞书《0-1项目验证》wiki；配套方法论 8 篇见《0-1项目提效验证》目录。
> 本文档所有案例数据来自真实项目记录（商机平台 / 工单系统 / 某政项目 / baseProject / web-tool），标 🖐️ 处建议读完亲手一试。
