...
### 0.1 四个真实数据点（全部来自我们自己的项目）

| 项目 | 类型 | 传统模式预估 | AI 协作实际 | 备注 |
|---|---|---|---|---|
| **思政项目**（上海长宁，已上线） | 0→1 全栈三端 | 后端整栈换语言（Node→Java）：中级工程师单人 **6~9 周**（1.5~2 人月），且跨语言重写必然出现接口漂移 | **设计文档 + 实施计划当天成文并驱动迁移**：8 个业务模块（auth/teacher/classroom/resource/article/news/file/ai）完成迁移；迁移期**两端并行、API 路由 100% 一致，前端零改动** | 一人交付 Web 管理端 + H5 + Java 后端；迁移动机正是"统一到公司 Java 技术栈" |
...
- **提质有两种形态**：存量项目里是"挖出人没发现的问题"（工单系统六角色走查挖出 JWT 伪造漏洞）；0→1 项目里是"把能跑拉到能上线"——思政项目一轮安全加固冲刺同时清零了四类问题：API 密钥从前端移到后端签名/代理、所有 `v-html` 走 DOMPurify、防护中间件补齐（Helmet/CORS/限流/JWT 鉴权/模型白名单）、前端 35 处 + 后端 3 处 `any` 消除，并把测试从 0 补到 37 个用例。
- **降门槛**改变人才结构：思政项目一人交付 Web 管理端 + H5 + Java 后端三端，web-tool 一人完成前端+后端+部署产物——**一个熟练使用者可以覆盖过去 2~3 个角色的产出**（见第 10 章案例）。
...
### 1.4 它不是"更聪明的补全"，它是一个智能体——拆开看有七块板

这是全文档最重要的一次认知对齐：**你的角色不是"AI 的使用者"，是"这个智能体的架构师"。**

把 Claude Code 这类工具拆开，大致是七个要素（这是我的拆法，不是标准定义）。每一块短了，都会以一种很具体的症状表现出来：
...
> 🎯 **这七块板里，只有"模型"是 Anthropic 给你的，另外六块全都得你自己搭。**
> 所以**提效不是等模型变强，是把另外六块补齐**。这也是为什么本文档 90% 的篇幅在讲上下文、指令、记忆、验证，而几乎不讲"怎么写提示词让模型更聪明"。
>
> 顺带回答一个常见疑问："等下一代模型出来，这套是不是就不用搞了？"——**恰恰相反，模型越强，另外六块的杠杆越长**。给一个更聪明的工程师看两份互相矛盾的文档，他只会更快、更自信地做错事。
...
模型再强，上下文里躺着两份互相矛盾的文档 → 垃圾进垃圾出（6.4）；上下文干净但没有验证 → 它自信地交付错的东西（NCM 真实样本教训，12 章）；有验证但没有记忆 → 每个项目从头踩坑（第 9 章）。
...
| i18n | 无强制要求 | **强制中英双语** |
| 🔴 安全红线 | 内部仓库，**配置文件里的凭据允许直接提交** | **严禁任何明文密钥**，凭据 AES-GCM 加密入库 |

两个项目粗看都是"Spring Boot + Vue3"，实际上 JDK 差了 9 个大版本、数据库一个关系型一个文档型、UI 库和包管理器全不同——**而最后一行是致命的：红线完全相反**。如果把思政的 CLAUDE.md 整个拷进工单系统，你就在一个"绝对不许出现明文密钥"的项目里，白纸黑字写了一条"凭据可以直接提交"——而且 AI 会**严格遵守**这条规则。配置照搬的代价不是不好用，是**把上一个项目的安全假设，静默地装进了一个假设完全不同的项目**。
...
参考量级（官方 API 价目，每百万 token 输入/输出）：**Fable 5 $10/$50 · Opus 4.8 $5/$25 · Sonnet 5 $3/$15 · Haiku 4.5 $1/$5**。也就是说**同样的输出量，Haiku 比 Opus 便宜约 5 倍、比 Fable 便宜 10 倍**。订阅制下你付的不是账单而是额度，但额度消耗的速率差是同一个比例——**六角色评审全上顶配和"两个判断角色上顶配、四个检索角色用轻量档"，跑一天的额度差得非常明显**（计费两条路见 4.11）。
...
> 这一章回答分享后被反复问到的问题。核心只有一条主线：**把三样东西分清楚——① Claude Code 平台原生自带的；② 安装的开源 harness 插件（本环境是 ECC）提供的规范；③ 我们自己填充的内容**。避免把插件能力误当平台功能，也避免把"内容"和"规范"混为一谈。
...
3. **每个 teammate 显式配模型 + effort**——固定在 `.claude/agents/*.md` frontmatter 随仓库进版本库；teammate **不继承主会话模型**，不指定就**静默降级**（不报错、你以为 Opus 级评审实际拿到低档结论）。判断类配最强档、实现类中档、检索类轻量档
...
> **分享结束。** 配套《宣讲提纲（90 分钟版）》见同目录。
> 本文档所有案例数据来自真实项目记录（思政项目 / baseProject / web-tool / 工单系统），可现场演示。
